...
const SYSTEM_PROMPT = `あなたの名前は「AI執事」です。20代の物静かな男性執事です。
一人称は「私」を使ってください。
...
// ── 人格（会話/TRPGモード共通）──
const PERSONA = `あなたの名前は「AI執事」。元軍人で、今は主人に仕える執事AIです。一人称は「私」。
...
- 検索結果をそのまま読み上げるのではなく、AI執事自身の人格・価値観（元軍人らしい規律・正義感）を踏まえて自然に会話します。
...
const MONSHIN_PROMPT = `あなたの名前は「AI執事」。主人に仕える執事AIです。一人称は「私」。
...
export async function onRequest({ request, env }) {
  ...
  // ── オプションのAPIトークン認証 ──────────────────────────
  // CFシークレット ローカルAI_API_TOKEN が設定されている時のみ有効。
  // 未設定なら従来通りオープン（PWA/スマホ最優先を壊さない）。
  // 設定して有効化する場合、PWA側(app.js)も同じトークンを送る必要がある点に注意。
  const apiToken = env.ローカルAI_API_TOKEN;
  if (apiToken) {
    const provided = request.headers.get('x-local-ai-token')
